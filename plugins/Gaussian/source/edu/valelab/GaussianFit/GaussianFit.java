...
public class GaussianFit {
	...
	/**
	 * Gaussian fit can be run by estimating parameter c (width of Gaussian) as
	 * 1 (circle), 2 (width varies in x and y), or 3 (ellipse) parameters
	 * 
	 * @param mode
	 *            - fit circle (1) ellipse(2), or ellipse with varying angle (3)
	 * @param fitmode
	 *            - algorithm use: NelderMead (1), or Levenberg Marquard (2)
	 */
	public GaussianFit(int mode, int fitMode) {
		...
		if (mode == 2) {
			mode_ = 2;
			paramNames_ = new String[] { "A", "b", "x_c", "y_c", "sigmaX",
					"sigmaY" };
		}
		if (mode == 3) {
			mode_ = 3;
	 ...
	 * for the fit and sends of to Apache fitting code Background is estimated
	 * by averaging the outer rows and columns Sigma estimate is hardcoded to
	 * 1.115 pixels Signal estimate is derived from total signal - total
	 * background estimate Steps sizes for the optimizer are set at 0.3 * the
	 * estimate values
	 * 
	 * @param siProc
	 *            - ImageJ ImageProcessor containing image to be fit
	 * @param maxIterations
	 *            - maximum number of iterations for the Nelder Mead
	...
	public double[] doGaussianFit(ImageProcessor siProc, int maxIterations) {
		...
		if (fitMode_ == 1) {
			...
			mGF_.setImage((short[]) siProc.getPixels(), siProc.getWidth(),
					siProc.getHeight());
			try {
				// RealPointValuePair result = nm_.optimize(mGF_,
				// GoalType.MINIMIZE, params0_);
				RealPointValuePair result = opt.optimize(maxIterations, mGF_,
						GoalType.MINIMIZE, params0_);
				paramsOut = result.getPoint();

				ReportingUtils.logMessage("!!!--- Evaluations: "
						+ opt.getEvaluations() + " / "
						+ opt.getMaxEvaluations() + ".");
		...
		if (fitMode_ == 2) {
			...
			try {
				paramsOut = cF.fit(maxIterations,
						new ParametricGaussianFunction(mode_,
								siProc.getWidth(), siProc.getHeight()),
						params0_);
		...
		if (mode_ == 3) {
			if (paramsOut.length > S3) {
				double[] prms = GaussianUtils.ellipseParmConversion(
						paramsOut[S1], paramsOut[S2], paramsOut[S3]);
				paramsOut[S1] = prms[1];
				paramsOut[S2] = prms[2];
				paramsOut[S3] = prms[0];
			}
	...
	private void estimateParameters(ImageProcessor siProc) {
		...
		for (int i = 0; i < siProc.getHeight() * siProc.getWidth(); i++) {
			...
			mx += ((int) (imagePixels[i] & 0xffff) - bg)
					* (i % siProc.getWidth());
			my += ((int) (imagePixels[i] & 0xffff) - bg)
					* (i / siProc.getWidth());
		}
		double ti = mt - (bg * siProc.getHeight() * siProc.getWidth());
