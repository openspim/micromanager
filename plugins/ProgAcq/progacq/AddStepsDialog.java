...
public class AddStepsDialog extends JDialog implements ActionListener {
	...
	public static void doInstance(JFrame parent, CMMCore core, Vector<String> devices, WindowListener listener)
	{
	...
	public AddStepsDialog(JFrame parent, CMMCore core, Vector<String> devices)
	{
		...
		for(String name : devices) {
			...
			try {
				...
				else if(core.getDeviceType(name).equals(DeviceType.XYStageDevice))
				{
		...
		{	
			...
			if(components.length == 3)
			{
				...
				try {
					...
					JOptionPane.showMessageDialog(this, "Invalid or malformed number.");
					results = null;
					if(focus != null)
						focus.grabFocus();
					return;
